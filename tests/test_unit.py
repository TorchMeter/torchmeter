...
def test_auto_unit(all_type_unit) -> None:
    ...
    for i in range(len(stage_vals) - 1):
        ...
        if 2 * low_stage < high_stage:
            ...
            int_multiple_time = 1
        assert f"{int_multiple_time} {unit}" == auto_unit(integral_multiple_val, unit_system=all_type_unit)
        ...
        assert f"{1.9:.2f} {unit}" == auto_unit(float_multiple_val, unit_system=all_type_unit)

    # out of range(smaller)
    underflow_float_val = stage_vals[0] / 2
    assert f"{underflow_float_val:.2f}" == auto_unit(underflow_float_val, unit_system=all_type_unit)

    underflow_int_val = int(underflow_float_val)
    assert f"{underflow_int_val}" == auto_unit(underflow_int_val, unit_system=all_type_unit)

    # out of range(bigger)
    unit = all_type_unit(stage_vals[-1]).name

    overflow_integral_multiple_val = stage_vals[-1] * 2
    assert f"2 {unit}" == auto_unit(overflow_integral_multiple_val, unit_system=all_type_unit)
