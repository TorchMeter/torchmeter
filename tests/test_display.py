...
class TestApplySetting:
    ...
    def test_setting(self) -> None:
        ...
        obj = NoVPAObj(1, 2, 3)
        apply_setting(obj, setting={"a": 1000, "b": 2000, "c": 3000}, c=30)
        assert obj._a == 1000
        assert obj._b == 2000
        assert obj._c == 30
    ...
    @pytest.mark.parametrize(
        ...
        argvalues=[
            ...
            (
                ...
                {"a": [7, 8, 9], "b": 40, "c": 50},
                "_a",
                {"_a": (1, 2, 3, 4, 5), "_b": 40, "_c": 50},
            ),
            (
                VPAMObj(1, 2, 3, 4, 5),
                {"a": 10, "b": [7, 8, 9], "c": 50},
                "_b",
                {"_a": 10, "_b": (2, 3, 4, 5), "_c": 50},
            ),
            ...
            (
                MixedArgsObj(2, 4, 6, 8, d=10, f=12),
                {"a": 10, "b": 20, "c": [1, 2, 3], "d": 40, "g": "G"},
                "_a",
                {"_a": 2, "_b": 20, "_c": (1, 2, 3), "_d": 40, "_e": {"g": "G"}},
            ),
            (
                MixedArgsObj(2, 4, 6, 8, d=10, f=12),
                {"a": 10, "b": 20, "c": [1, 2, 3], "d": 40, "g": "G"},
                "_c",
                {"_a": 10, "_b": 20, "_c": (6, 8), "_d": 40, "_e": {"g": "G"}},
            ),
            (
                MixedArgsObj(2, 4, 6, 8, d=10, f=12),
                {"a": 10, "b": 20, "c": [1, 2, 3], "d": 40, "g": "G"},
                "_d",
                {"_a": 10, "_b": 20, "_c": (1, 2, 3), "_d": 10, "_e": {"g": "G"}},
            ),
            (
                MixedArgsObj(2, 4, 6, 8, d=10, f=12),
                {"a": 10, "b": 20, "c": [1, 2, 3], "d": 40, "g": "G"},
                "_e",
                {"_a": 10, "_b": 20, "_c": (1, 2, 3), "_d": 40, "_e": {"f": 12}},
            ),
...
class TestRenderPerline:
    ...
    def test_instant_render(self, mock_config, mock_console, monkeypatch) -> None:
        ...
        with patch("rich.get_console", return_value=mock_console), patch("time.sleep") as mock_sleep:
    ...
    def test_render_line_by_line(self, mock_config, mock_console, monkeypatch) -> None:
        ...
        with patch("rich.get_console", return_value=mock_console), patch("time.sleep") as mock_sleep:
            ...
            mock_console.render_lines.assert_called_once_with("multi\nline\ncontent", new_lines=True)
    ...
    def test_various_content(self, content, render_lines_num, mock_console, monkeypatch) -> None:
        """Test handling empty renderable content"""

        with patch("rich.get_console", return_value=mock_console), \
             patch("time.sleep") as mock_sleep:  # fmt: skip
            # no time interval
            monkeypatch.setattr("torchmeter.display.__cfg__.render_interval", 0)
...
class TestTreeRenderer:
    ...
    def test_default_level_args(self, simple_tree_renderer) -> None:
        ...
        assert all(
            hasattr(default_args, f)
            for f in [
                "label",
                "style",
                "guide_style",  # define in display.py::TreeRenderer::default_level_args
                "highlight",
                "hide_root",
                "expanded",
            ]
        ...
        with pytest.raises(KeyError):
            simple_tree_renderer.default_level_args = {"invalid_field": "value", "label": "test"}
        ...
        default_args.mark_unchange()
        simple_tree_renderer.default_level_args = {"label": "test", "style": "magenta"}
        assert default_args.is_change()
    ...
    def test_tree_levels_args(self, simple_tree_renderer) -> None:
        ...
        # with combination of invalid field and valid field
        with pytest.raises(KeyError):
            simple_tree_renderer.tree_levels_args = {"default": {"invalid_field": "value", "label": "test"}}
        ...
        levels_args.mark_unchange()
        simple_tree_renderer.tree_levels_args = {
            "default": {"guide_style": "blue"},
            "3": {"label": "label 3", "style": "green", "guide_style": "red"},
        }
        assert levels_args.is_change()
    ...
    def test_repeat_block_args(self, simple_tree_renderer) -> None:
        ...
        # with combination of invalid field and valid field
        with pytest.raises(KeyError):
            simple_tree_renderer.repeat_block_args = {"invalid_field": "value", "border_style": "yellow"}
    ...
    def test_default_footer(self, monkeypatch) -> None:
        ...
        class RepeatWinszModel(nn.Module):
            def __init__(self, repeat_winsz=1, repeat_time=3) -> None:
                ...
                self.layer = nn.ModuleList(repeat_time * sample(candidate_layers, repeat_winsz))
    ...
    def test_isolated_rendering(self, repeat_tree_renderer, monkeypatch) -> None:
        ...
        assert all(c.node_id == f"2.{c_idx + 1}" 
                   for c_idx, c in enumerate(oproot.childs["2"].childs.values()))  # fmt: skip
    ...
    def test_node_id_generation(self, repeat_tree_renderer, monkeypatch) -> None:
        ...
        assert all(isinstance(c, Tree) for c in [child_1, child_2, child_2_1, child_2_2, child_2_3, child_2_4])
        assert child_1.label == "1"
        assert child_2.label == "2"
    ...
    def test_style_application(self, repeat_tree_renderer, monkeypatch) -> None:
        ...
        repeat_tree_renderer.tree_levels_args = {
            "default": {"label": "[<node_id>] <name>-<type>", "guide_style": "red"},
            "0": {"label": "<name>", "style": "magenta"},
            "1": {"label": "<node_id>", "guide_style": "blue"},
        }

        repeat_tree_renderer.repeat_block_args = {"title": "test title", "style": "cyan"}
...
class TestTabularRenderer:
    ...
    def test_tb_args(self, simple_tabular_renderer) -> None:
        ...
        # with combination of invalid field and valid field
        with pytest.raises(KeyError):
            simple_tabular_renderer.tb_args = {"invalid_field": "value", "border_style": "yellow"}
    ...
    def test_col_args(self, simple_tabular_renderer) -> None:
        ...
        # with combination of invalid field and valid field
        with pytest.raises(KeyError):
            simple_tabular_renderer.col_args = {"invalid_field": "value", "no_wrap": True}
        ...
        # update several settings
        col_args.mark_unchange()
        simple_tabular_renderer.col_args = {"style": "red", "vertical": "top"}
        assert col_args.is_change()
    ...
    def test_export(self, simple_tabular_renderer, example_df, export_dir) -> None:
        ...
        with pytest.raises(ValueError):
            simple_tabular_renderer.export(df=example_df, save_path=export_dir)

        # extension is unsupported
        with pytest.raises(ValueError):
            simple_tabular_renderer.export(
                df=example_df,
    ...
    def test_call_data_acquisition(self, mock_dfs_task, simple_tabular_renderer, universal_tabular_renderer) -> None:
    ...
    def test_call_pick_col(self, universal_tabular_renderer) -> None:
        ...
        # valid usage
        _, data1 = universal_tabular_renderer(
            stat_name="param",
            pick_cols=[
                "Operation_Id",
                "Operation_Name",
        ...
        _, data2 = universal_tabular_renderer(
            stat_name="param",
            pick_cols=[
                "Numeric_Num",
                "Param_Name",
        ...
        # invalid column name
        with pytest.raises(ValueError):
            universal_tabular_renderer(stat_name="param", pick_cols=["invalid_col"])
    ...
    def test_call_exclude_col(self, universal_tabular_renderer) -> None:
        ...
        _, data = universal_tabular_renderer("param", exclude_cols=["Operation_Type", "Operation_Name"])
    ...
    def test_call_custom_col(self, universal_tabular_renderer) -> None:
        ...
        # basic usage
        _, data = universal_tabular_renderer(
            stat_name="param",
        ...
        assert all(col_name not in data.columns 
                   for col_name in ["Operation_Id", "Operation_Name", "Numeric_Num"])  # fmt: skip
        assert all(col_name in data.columns 
                   for col_name in ["Operation Id", "Operation Name", "Numeric Value"])  # fmt: skip
    ...
    def test_call_pick_exclude_cooperation(self, universal_tabular_renderer) -> None:
        ...
        _, data = universal_tabular_renderer(
            stat_name="param",
            pick_cols=["Operation_Id", "Param_Name", "Numeric_Num"],
    ...
    def test_call_pick_custom_cooperation(self, universal_tabular_renderer) -> None:
        ...
        _, data = universal_tabular_renderer(
            stat_name="param",
            pick_cols=["Operation_Id", "Param_Name"],
